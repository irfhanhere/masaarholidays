...
import { createManualBookingVoucher, createDocumentFromSource } from "@/app/admin/(dashboard)/documents/actions";

export async function POST(req: Request) {
  try {
    const body = await req.json();
    if (body.source_id) {
      const result = await createDocumentFromSource(body.source_id, "booking_voucher");
      if (!result.success) {
    ...
    const result = await createManualBookingVoucher(body);
    if (!result.success) {
